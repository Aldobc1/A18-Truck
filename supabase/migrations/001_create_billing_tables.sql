...
CREATE TABLE IF NOT EXISTS payment_methods_a18 (
    ...
    -- Solo un método de pago por defecto por workspace
    CONSTRAINT unique_default_payment_method EXCLUDE (workspace_id WITH =) WHERE (is_default = TRUE)
);
...
COMMENT ON COLUMN subscriptions_a18.plan_id IS 'ID del plan: basic, business, professional';
COMMENT ON COLUMN subscriptions_a18.status IS 'Estado de la suscripción: active, cancelled, past_due, pending, incomplete';
COMMENT ON COLUMN invoices_a18.status IS 'Estado de la factura: paid, pending, failed, cancelled';
COMMENT ON COLUMN payment_methods_a18.is_default IS 'Indica si este es el método de pago por defecto para el workspace';