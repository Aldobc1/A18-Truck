              ...
              <div className="hidden sm:block mr-2">
                <div className="relative">
                  <button 
                    ...
                    className="flex items-center justify-between px-3 py-1.5 bg-purple-50 rounded-lg hover:bg-purple-100 transition-colors"
                  >
                    <div className="flex items-center space-x-2">
                    ...
                    <SafeIcon 
                      icon={FiChevronDown} 
                      className={`w-4 h-4 text-gray-500 transition-transform ml-2 ${showDashboardDropdown ? 'transform rotate-180' : ''}`} 
                    />
                  </button>
                    ...
                    <div className="absolute right-0 z-10 mt-1 w-48 bg-white rounded-md shadow-lg border border-gray-200 py-1">
                      {dashboards.map((dashboard) => (
                        ...
                        >
                          <SafeIcon 
                            icon={dashboard.icon} 
                            className={`w-4 h-4 ${location.pathname.startsWith(dashboard.href) ? 'text-purple-600' : 'text-gray-400'}`} 
                          />
        ...
        {showMobileMenu && (
          ...
          >
            <div className="flex flex-col space-y-2">
              <button
                onClick={() => { setActiveView('register'); setShowMobileMenu(false); }}
                className={`px-3 py-2 rounded-lg font-medium transition-colors flex items-center 
                  ${activeView === 'register' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}
              >
              ...
              <button
                ...
                className={`px-3 py-2 rounded-lg font-medium transition-colors flex items-center 
                  ${activeView === 'history' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}
              >
              ...
              {isAdmin && (
                <div className="relative">
                  <button
                    onClick={() => setShowDashboardDropdown(!showDashboardDropdown)}
                    className="w-full px-3 py-2 rounded-lg font-medium transition-colors flex items-center justify-between bg-purple-50 text-gray-800"
                  >
                    <div className="flex items-center">
                    ...
                    <SafeIcon
                      icon={FiChevronDown}
                      className={`w-4 h-4 text-gray-500 transition-transform ${showDashboardDropdown ? 'transform rotate-180' : ''}`}
                    />
                  </button>
                  {showDashboardDropdown && (
              ...
              <button
                onClick={logout}
                className="px-3 py-2 rounded-lg font-medium transition-colors flex items-center text-red-600 bg-red-50"
              >
        ...
        <div className="hidden sm:flex items-center justify-between mt-2">
          <div className="flex bg-gray-100 rounded-lg p-1">
            <button
              ...
              className={`px-4 py-2 rounded-md font-medium transition-colors ${activeView === 'register' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}
            >
            ...
            <button
              ...
              className={`px-4 py-2 rounded-md font-medium transition-colors ${activeView === 'history' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}
            >
              ...
              >
                ...
                <div className="space-y-3">
                  ...
                  <motion.button
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    onClick={() => { setShowManualSelect(true); setStep('selectTruck'); }}
                    className="w-full bg-gray-600 text-white py-2.5 rounded-lg font-medium hover:bg-gray-700 transition-colors text-sm"
                  >
                      ...
                      <div className="flex items-center space-x-1">
                        <span
                          className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium ${
                            record.isCompleted ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'
                          }`}
                        >
                          ...
                          <span>
                            ...
                            {record.truck?.estimatedTravelTime && (
                              <span className="text-gray-500">
                                {' '}(Est: {record.truck.estimatedTravelTime} min)
                              </span>
                            )}
                          </span>
                        </div>
      ...
      {activeView === 'register' && step === 'scan' && (
        <div className="fixed bottom-6 right-6 z-10">
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={() => setShowNFCReader(true)}
            className="w-14 h-14 bg-blue-600 rounded-full shadow-lg flex items-center justify-center"
          >
